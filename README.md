...
Both vaults distinguish accepted change requests from verified lock states. Keeper and final-inventory checks retry an unconfirmed read up to three times, waiting 0.5, 1, and 2 seconds; they never resend a write automatically. Keeper verification checks the exact instance ID as well as its lock bit. If individual-item reads remain unconfirmed, a fresh inventory read can confirm the keeper after the entire item family is revalidated against the preview, accounting for this batch's accepted writes.

An unconfirmed keeper now skips only its group's duplicate unlocks and lets other groups continue. The result lists skipped groups, protected duplicates, accepted requests, and individually verified changes; it does not claim the full batch succeeded. The checklist is still saved. API failures, account changes, invalid inventory, and changed item families continue to stop the batch. Final mismatches list the expected and observed states. Run a fresh scan before retrying.
