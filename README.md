...
For cached batch confirmation, use `generated-weapon.html?weapon=exotic&lock-cache=1&fast-waits=1`: seven unchanged inventory reads produce a pending result, then Check current lock states sees the simulated updated locks. Browser QA verified the pending display, recheck to verified without writes, persistence after reload, deferred duplicate unlocks, and the shared armor flow. The test-only fast-waits parameter skips delays without changing production timing.
