...
Both vaults distinguish accepted change requests from verified lock states. Keeper and final-inventory checks retry an unconfirmed read up to three times, waiting 0.5, 1, and 2 seconds; they never resend a write automatically. Keeper verification checks the exact instance ID as well as its lock bit. Duplicate unlocks still wait for the keeper, and a failed request or persistent verification failure stops the batch. Errors name the item, location, instance ID, and operation phase; final mismatches list the expected and observed states.

The last lock result is retained in this browser, separately for each vault, and remains visible after reload or another scan. It shows accepted versus planned requests, verified lock/unlock counts, excluded review copies, and any failure. **Export last lock result** exports these details without credentials. An interrupted attempt remains explicitly unverified. Results for a different selected Destiny account are hidden. If browser storage cannot retain the result, the app offers an export and shows the storage problem.
...
For lock verification, add `&lock-delay=1` to a weapon fixture for delayed keeper/final reads, or `&lock-fail=1` for an API failure while unlocking a duplicate. `?weapon=exotic` uses exotic weapons without standard perk columns. Browser QA verified successful retries, zero remaining changes after rescan, named partial failure surviving reload, exotic locks, and verified armor locks followed by a separate checklist storage failure.
